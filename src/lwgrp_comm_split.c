...
static int lwgrp_sort_bitonic_merge(
  int value[3], int start, int num, int direction, const lwgrp_chain* group, const lwgrp_logchain* list)
{
  int scratch[3];
  ...
  if (num > 1) {
    ...
    if (rank < start + count) {
      ...
      if (dst_rank < start + num) {
        ...
        MPI_Sendrecv(
          value,   3, MPI_INT, partner, 0,
          scratch, 3, MPI_INT, partner, 0,
          comm, status
        );
      ...
      lwgrp_sort_bitonic_merge(value, start, count, direction, group, list);
    } else {
      int dst_rank = rank - count;
      if (dst_rank >= start) {
        /* exchange data with our partner rank */
      ...
      lwgrp_sort_bitonic_merge(value, new_start, new_num, direction, group, list);
    }
  }
  ...
  int value[3], int start, int num, int direction, const lwgrp_chain* group, const lwgrp_logchain* list)
{
  if (num > 1) {
    ...
    int mid = num / 2;
    if (rank < start + mid) {
      lwgrp_sort_bitonic_sort(value, start, mid, !direction, group, list);
    } else {
      int new_start = start + mid;
      int new_num   = num - mid;
      lwgrp_sort_bitonic_sort(value, new_start, new_num, direction, group, list);
    }

    /* merge the two sorted halves */
    lwgrp_sort_bitonic_merge(value, start, num, direction, group, list);
  }
...
static int lwgrp_logchain_sort_bitonic(int item[3], const lwgrp_chain* group, const lwgrp_logchain* list)
{
  /* conduct the bitonic sort on our values */
  int ranks = group->group_size;
  int rc = lwgrp_sort_bitonic_sort(item, 0, ranks, 1, group, list);
  return rc;
}
...
static int lwgrp_chain_split_sorted(const int val[3], const lwgrp_chain* in, lwgrp_chain* out)
{
  int k;
  ...
  if (left_rank != MPI_PROC_NULL) {
    MPI_Isend((void*)val, 3, MPI_INT, left_rank, 0, comm, &request[k]);
    k++;

    MPI_Irecv(left_buf, 3, MPI_INT, left_rank, 0, comm, &request[k]);
    k++;
  }
  if (right_rank != MPI_PROC_NULL) {
    MPI_Isend((void*)val, 3, MPI_INT, right_rank, 0, comm, &request[k]);
    k++;

    MPI_Irecv(right_buf, 3, MPI_INT, right_rank, 0, comm, &request[k]);
    k++;
  }
  if (k > 0) {
  ...
  while (left_rank != MPI_PROC_NULL || right_rank != MPI_PROC_NULL) {
    ...
    if (left_rank != MPI_PROC_NULL) {
      MPI_Irecv(recv_left_ints, 3, MPI_INT, left_rank, 0, comm, &request[k]);
      k++;
      ...
      MPI_Isend(send_left_ints, 3, MPI_INT, left_rank, 0, comm, &request[k]);
      k++;
    }

    /* send and receive data with right partner */
    if (right_rank != MPI_PROC_NULL) {
      MPI_Irecv(recv_right_ints, 3, MPI_INT, right_rank, 0, comm, &request[k]);
      k++;
      ...
      MPI_Isend(send_right_ints, 3, MPI_INT, right_rank, 0, comm, &request[k]);
      k++;
    }
  ...
  int recv_group_ints[4];
  MPI_Isend(send_group_ints, 4, MPI_INT, val[CKR_RANK],  1, comm, &request[0]);
  MPI_Irecv(recv_group_ints, 4, MPI_INT, MPI_ANY_SOURCE, 1, comm, &request[1]);
 ...
 * of the output group */
int lwgrp_comm_split_chain(MPI_Comm comm, int color, int key, lwgrp_chain* out)
{
  ...
  lwgrp_logchain_sort_bitonic(item, &in, &list);
  ...
  lwgrp_chain_split_sorted(item, &in, out);
...
int lwgrp_comm_split_members(MPI_Comm comm, int color, int key, int* size, int members[])
{
   ...
   * to bitonic sort */
  lwgrp_chain out;
  lwgrp_comm_split_chain(comm, color, key, &out);
 ...
 * MPI-2.2 and later */
int lwgrp_comm_split_create(MPI_Comm comm, int color, int key, MPI_Comm* new_comm)
{
